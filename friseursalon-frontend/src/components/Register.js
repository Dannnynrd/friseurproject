...
const RegisterSchema = Yup.object().shape({
    ...
    password: Yup.string().required('Passwort ist erforderlich.').min(6, 'Passwort muss mindestens 6 Zeichen lang sein.').max(40, 'Passwort darf maximal 40 Zeichen lang sein.'),
...
function Register() { // onRegisterSuccess Prop entfernt, da wir immer zum Login navigieren
    ...
    const handleRegister = (formValue, { setSubmitting }) => {
        ...
        AuthService.register(firstName, lastName, email, password, phoneNumber || null).then(
            (response) => {
                // KORREKTUR: Sofortige Weiterleitung zum Login mit einer Erfolgsmeldung im State.
                navigate('/login', {
                    state: { message: "Registrierung erfolgreich! Sie können sich jetzt anmelden." }
                });
            },
            (error) => {
                    ...
                    {({ errors, touched, isSubmitting }) => (
                        <Form className="mt-8 space-y-6">
                            ...
                            <div>
                                ...
                                >
                                    {loading || isSubmitting ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faUserPlus} className="mr-2 h-5 w-5 text-indigo-300 group-hover:text-indigo-200" />}
                                    Konto erstellen
                                </button>
                            </div>
                            {error && (
